...
#pragma region Vars
		...
		double *dol = new double[7];

		dol[4] = 7 * DK; dol[5] = 14 * DK;  dol[6] = 23 * DK;
		dol[3] = 0;
		dol[2] = -7 * DK; dol[1] = -14 * DK;  dol[0] = -23 * DK;

		double **Line = new double*[50];
		for(int32_t i = 0; i < 50; i++) Line[i] = new double[7];
...
#pragma endregion
		...
		for(int32_t u = 1, totu = 0, breaked = 0; u < iterc; u++, totu++)
		{
			...
			if(u % 2 == 0)
			{
				r = tr[int32_t(min(0, max((count * 1999) / sublenght, 1999)))];
		...
		try
		{
			...
			if(Krit < -1e10)
			{
				for(int32_t i = 0; i < 50; i++) delete[] Line[i];
				delete[] Line;
				delete[] dol;
				SheetFound = 0;
...
#pragma endregion
	...
	float GetKrit2(uint8_t **img, Vector2 A, Vector2 B, Vector2 C, Vector2 D)
	{
...
#define Oreintdisp 40
		float proectednoga[200][4];
		for(int32_t i = 0; i < 200; i++)
		{
			for(int32_t j = 0; j < 4; j++)
				proectednoga[i][j] = shariki[i][j];
		}
...
#pragma region
		...
		double xAb = lstat2[0].xA;
		double xBb = lstat2[0].xB;
		double xCb = lstat2[0].xC;
		double xDb = lstat2[0].xD;

		double yAb = lstat2[0].yA;
		double yBb = lstat2[0].yB;
		double yCb = lstat2[0].yC;
		double yDb = lstat2[0].yD;
...
#pragma region
		...
		xAb = lstat.xA; yAb = lstat.yA;
		xBb = lstat.xB; yBb = lstat.yB;
		xCb = lstat.xC; yCb = lstat.yC;
		xDb = lstat.xD; yDb = lstat.yD;
...
#pragma region Do poisk
		...
		{
			out = new double*[30];
			for(int32_t i = 0; i < 30; i++) { out[i] = new double[10]; for(int32_t j = 0; j < 10; j++) out[i][j] = 0; }
		}
